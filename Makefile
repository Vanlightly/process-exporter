...
PREFIX                  ?= $(shell pwd)
BIN_DIR                 ?= $(shell pwd)
DOCKER_IMAGE_NAME       ?= jackvanlightly/process-exporter
...
dockertest:
	docker run --rm -it -v `pwd`:/go/src/github.com/vanlightly/process-exporter golang:1.12  make -C /go/src/github.com/vanlightly/process-exporter test

dockerinteg:
	docker run --rm -it -v `pwd`:/go/src/github.com/vanlightly/process-exporter golang:1.12  make -C /go/src/github.com/vanlightly/process-exporter build integ
